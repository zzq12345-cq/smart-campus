<template>
  <view :class="['market-page', themeClass]">
    <view class="top-bar">
    ...
    <view v-else-if="!items.length" class="state-card">
      <Icon name="storefront" :size="40" color="rgba(244,157,37,0.3)" />
...
import type { MarketItem, MarketItemCategory } from '@/types/market-item'
import { useAuthStore } from '@/stores/auth'
import { useUiPreferencesStore } from '@/stores/ui-preferences'
import { requireAuth } from '@/utils/auth-guard'
import MarketCard from '@/components/life/MarketCard.vue'

type CategoryFilter = MarketItemCategory | 'all'
const PAGE_SIZE = 20
...
const pageTitle = computed(() => (isZh.value ? '二手市场' : 'Second-hand Market'))
const searchPlaceholder = computed(() => (isZh.value ? '搜索商品...' : 'Search items...'))
const cancelText = computed(() => (isZh.value ? '取消' : 'Cancel'))
const loadingText = computed(() => (isZh.value ? '加载中...' : 'Loading...'))
const emptyText = computed(() => (isZh.value ? '暂无商品，快来发布第一个吧' : 'No items yet. Be the first to post!'))
const loadMoreText = computed(() => (isZh.value ? '加载更多' : 'Load more'))

const categories = computed(() => [
  { value: 'all' as CategoryFilter, label: isZh.value ? '全部' : 'All' },
  { value: 'digital' as CategoryFilter, label: isZh.value ? '数码' : 'Digital' },
  { value: 'textbook' as CategoryFilter, label: isZh.value ? '教材' : 'Textbook' },
  { value: 'daily' as CategoryFilter, label: isZh.value ? '生活' : 'Daily' },
  { value: 'clothing' as CategoryFilter, label: isZh.value ? '服装' : 'Clothing' },
  { value: 'sports' as CategoryFilter, label: isZh.value ? '运动' : 'Sports' },
  { value: 'other' as CategoryFilter, label: isZh.value ? '其他' : 'Other' }
])
...
const loadItems = async (reset = true) => {
  ...
  try {
    ...
    const result = await marketItemsService.getItems({
    ...
    console.error('Load market items failed:', error)
    uni.showToast({
...
const doSearch = async () => {
  ...
  hasMore.value = false
  try {
    items.value = await marketItemsService.searchItems(keyword)
  } catch (error) {
    console.error('Search market items failed:', error)
    uni.showToast({
...
const goDetail = (id: string) => {
  uni.navigateTo({
    url: `/pages/life/market/detail?id=${encodeURIComponent(id)}`
  })
}

const goPublish = () => {
  if (!requireAuth('/pages/life/market/index')) return
  uni.navigateTo({ url: '/pages/life/market/publish' })
}
