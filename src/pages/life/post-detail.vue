<template>
  <view :class="['life-detail-page', themeClass]">
    <view class="top-bar">
    ...
    <view class="hero-strip">
      <view class="hero-inner">
        <Icon name="self_improvement" :size="24" color="#f49d25" />
    ...
    <view v-else-if="post" class="detail-wrap">
      <view class="post-card">
        ...
        <view class="post-footer">
          ...
          <view class="action-row">
            ...
            >
              <Icon name="mail" :size="16" color="#f49d25" />
      ...
      <view class="section-title-row">
        <view class="section-left">
          <Icon name="chat" :size="18" color="#f49d25" />
      ...
      <view class="comment-editor">
        ...
        <view class="comment-editor-row">
          <view class="comment-anon">
            ...
            <switch :checked="isAnonymousComment" @change="onAnonymousChange" color="#f49d25" />
          </view>
          ...
          <view class="comment-head">
            <view class="comment-author">
              <view class="comment-avatar">
                <image v-if="item.avatar" :src="item.avatar" class="avatar-image" mode="aspectFill" lazy-load />
...
const heroTitle = computed(() => (isZh.value ? '生活信息详情' : 'Life discussion'))
const heroSubtitle = computed(() => (isZh.value ? '聚焦互助、交易与活动' : 'Help, trading, and activities'))
...
const topicLabel = (topic?: string) => {
  const map: Record<LifeTopic, { zh: string; en: string }> = {
    life_help: { zh: '生活互助', en: 'Mutual Help' },
    second_hand: { zh: '二手交易', en: 'Second-hand Trade' },
    activity: { zh: '校园活动', en: 'Campus Events' },
    job: { zh: '就业招聘', en: 'Jobs' },
    rental: { zh: '租房信息', en: 'Housing' }
  }
  if (!isLifeTopic(topic)) {
    return isZh.value ? '生活动态' : 'Life Updates'
  }
...
const resolveAuthorProfiles = async (ids: string[]) => {
  ...
  try {
    ...
    console.error('Resolve life author profiles failed:', error)
  }
}
...
const loadComments = async () => {
  ...
  try {
    ...
    console.error('Load life comments failed:', error)
    comments.value = []
  } finally {
...
const loadDetail = async () => {
  ...
  try {
    ...
    if (rawPost.section && rawPost.section !== 'life') {
      throw new Error(isZh.value ? '该帖子不属于生活区' : 'Post section mismatch')
    }
    ...
    if (authStore.isLoggedIn) {
      try {
        ...
        console.error('Load life detail interaction state failed:', error)
      }
    }
    ...
    console.error('Load life post detail failed:', error)
    const message = extractErrorMessage(error)
...
.hero-strip {
  ...
  border: 1px solid var(--line);
  background:
    radial-gradient(circle at 80% 0%, rgba(244, 157, 37, 0.3) 0%, rgba(244, 157, 37, 0) 62%),
    linear-gradient(135deg, rgba(244, 157, 37, 0.2), rgba(244, 157, 37, 0.07));
  padding: 26rpx 24rpx;
}
...
.action-item.message-action {
  border-color: rgba(244, 157, 37, 0.28);
...
.comment-submit {
  padding: 12rpx 18rpx;
  border-radius: 999rpx;
  background: rgba(244, 157, 37, 0.2);
  color: #b45309;
  font-size: 24rpx;
  font-weight: 700;
}
