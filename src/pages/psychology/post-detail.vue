<template>
  <view :class="['psychology-detail-page', themeClass]">
    <view class="top-bar">
    ...
    <view v-else-if="post" class="detail-wrap">
      <view class="post-card">
        <view class="post-meta">
          <view class="meta-tags">
            <view class="meta-chip topic">{{ post.topicLabel }}</view>
        ...
        <view class="post-footer">
          ...
          <view class="action-row">
            ...
            >
              <Icon name="mail" :size="16" color="#886fde" />
      ...
      <view class="section-title-row">
        <view class="section-left">
          <Icon name="chat" :size="18" color="#886fde" />
      ...
      <view class="comment-editor">
        ...
        <view class="comment-editor-row">
          <view class="comment-anon">
            ...
            <switch :checked="isAnonymousComment" @change="onAnonymousChange" color="#886fde" />
          </view>
...
const heroTitle = computed(() => (isZh.value ? '心理区详情' : 'Psychology detail'))
const heroSubtitle = computed(() => (isZh.value ? '情绪支持与温暖互动' : 'Supportive and warm discussion'))
...
const isPsychologyTopic = (topic?: string): topic is PsychologyTopic =>
  Boolean(topic && (PSYCHOLOGY_TOPICS as readonly string[]).includes(topic))
...
const moodLabel = (mood?: string) => {
  ...
  return map[mood as keyof typeof map] || mood
}

const riskToneClass = (riskLevel?: number): 'low' | 'mid' | 'high' => {
...
const resolveAuthorProfiles = async (ids: string[]) => {
  ...
  try {
    ...
    console.error('Resolve psychology author profiles failed:', error)
  }
}
...
const mapPost = (raw: Post): DetailPostItem => {
  ...
  return {
    ...
    topicLabel: topicLabel(String(raw.topic || 'daily')),
    moodLabel: moodLabel(String(raw.mood || '')),
    riskLabel: riskLabel(Number(raw.riskLevel || 1)),
...
const loadComments = async () => {
  ...
  try {
    ...
    console.error('Load psychology comments failed:', error)
    comments.value = []
  } finally {
...
const loadDetail = async () => {
  ...
  try {
    ...
    if (rawPost.section && rawPost.section !== 'psychology') {
      throw new Error(isZh.value ? '该帖子不属于心理区' : 'Post section mismatch')
    }
    ...
    if (authStore.isLoggedIn) {
      try {
        ...
        console.error('Load psychology detail interaction state failed:', error)
      }
    }
    ...
    console.error('Load psychology post detail failed:', error)
    const message = extractErrorMessage(error)
...
.theme-dark {
  --page-bg: #1f1736;
  --surface: rgba(24, 20, 44, 0.8);
  --text-main: #f1edff;
  --text-sub: #d1d5db;
  --text-soft: #9ca3af;
  --line: rgba(136, 111, 222, 0.32);
  --topbar-bg: rgba(31, 23, 54, 0.9);
}
...
.hero-strip {
  ...
  border: 1px solid var(--line);
  background:
    radial-gradient(circle at 80% 0%, rgba(136, 111, 222, 0.3) 0%, rgba(136, 111, 222, 0) 64%),
    linear-gradient(135deg, rgba(136, 111, 222, 0.22), rgba(136, 111, 222, 0.08));
  padding: 26rpx 24rpx;
}
...
.action-item.message-action {
  border-color: rgba(136, 111, 222, 0.28);
...
.retry-btn {
  border-radius: 999rpx;
  border: 1px solid rgba(136, 111, 222, 0.32);
  padding: 10rpx 24rpx;
  background: rgba(136, 111, 222, 0.14);
}
