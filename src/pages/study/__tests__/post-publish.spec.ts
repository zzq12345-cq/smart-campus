...
import StudyPostPublishPage from '@/pages/study/post-publish.vue'
...
function mountPage() {
  return mount(StudyPostPublishPage, {
    global: {
      stubs: {
        Icon: {
          props: ['name'],
...
describe('study post publish page', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    vi.useFakeTimers()
    mockPostsService.createPost.mockResolvedValue({ $id: 'post-study-1' })
    mockUploadPostImage.mockResolvedValue('https://img.example.com/study-1.jpg')
  ...
  it('publishes post and navigates back', async () => {
    const wrapper = mountPage()
    await wrapper.find('.composer-input').setValue('学习区发帖测试内容')
    await wrapper.find('.submit-btn').trigger('tap')
    await flushPromises()
    vi.runAllTimers()
  ...
  it('uploads selected images before publish', async () => {
    ;(globalThis as any).uni.chooseImage = vi.fn(({ success }) =>
      success?.({
        tempFilePaths: ['/tmp/study-image.jpg'],
        tempFiles: [{ path: '/tmp/study-image.jpg' }]
      })
    )
    ...
    expect(mockPostsService.createPost).toHaveBeenCalledWith({
      ...
      isAnonymous: false,
      status: 'published',
      images: ['https://img.example.com/study-1.jpg']
    })
  })
})
